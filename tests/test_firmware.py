...
from herdrkeys.model import MIC, REPO, REPO_SLOT
...
def test_the_repo_key_answers_to_the_character_the_host_sends(firmware):
    # Same drift risk as the microphone key: two constants, two files.
    assert firmware["REPO_CODE"] == REPO


def test_the_repo_key_is_steady_and_unlike_every_other_feature_key(firmware):
    now = time.monotonic()
    repo = firmware["colour_for"](REPO, now, REPO_SLOT)
    ...
    over_a_cycle = {firmware["colour_for"](REPO, now + t, REPO_SLOT) for t in (0.0, 0.3, 0.7, 1.1, 2.3)}
...
def test_a_flash_naming_a_key_flashes_only_that_key(firmware):
    ...
    assert firmware["colour_for"](REPO, now, REPO_SLOT) == firmware["FN_FLASH"]
...
def test_a_flash_naming_nothing_still_flashes_the_function_key(firmware):
    ...
    assert firmware["colour_for"](REPO, now, REPO_SLOT) == firmware["REPO_IDLE"]
